...
Application::Application(int &argc, char **argv)
    : QApplication(argc, argv), settings_(0) {
  QtJsonSettings::initialize();
  QSettings::setDefaultFormat(QtJsonSettings::format());
  init();
}
...
void Application::init() {
  ...
  // static boost::shared_ptr<QTranslator> qt_trans = system_translator(
  // app_settings().country_name() );

  // if ( !qt_trans->isEmpty() )
  {
    // LOG_DEBUG << "Qt translation \"%1\" loaded",
    // app_settings().country_name();
  }
  // else
  {
    // LOG_DEBUG << "Failed to load Qt translation \"%1\". Trying \"%2\"
    // instead...", app_settings().country_name(), QLocale::system().name();

    // qt_trans = system_translator( QLocale::system().name() );
    // if( !qt_trans->isEmpty() )
    // LOG_DEBUG << "Qt translation \"%1\" loaded", QLocale::system().name();
    // else
    // LOG_DEBUG << "Failed to load Qt translation \"%1\"",
    // QLocale::system().name();
  }

  // a->installTranslator(qt_trans.get());

  // static boost::shared_ptr<QTranslator> urt_tr = local_translator(
  // app_settings().country_name() );

  // if ( !urt_tr->isEmpty()  )
  {
    // LOG_DEBUG << "Application translation \"%1\" loaded",
    // app_settings().country_name();
  }
  // else
  {
    // LOG_DEBUG << "Failed to load application translation \"%1\"",
    // app_settings().country_name();
  }
...
Settings &Application::settings() {
  if (!settings_) {
    settings_ = new Settings(QSettings::UserScope, organizationName(),
