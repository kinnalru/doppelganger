...
int main(int argc, char *argv[]) {
  ...
  QList<Replicas::Unit *> replicas;
  replicas.append(r1);
  replicas.append(r2);

  for (Replicas::Unit *r : replicas) {
    qDebug() << QJsonDocument(r->to_json()).toJson();
    controller->add_replica(r);
    w.add_replica(r);
  }

  QObject::connect(controller, &ReplicasController::refreshed, &w,
